...
//人脸检测状态 VIDEO模式
#define STATUS_INVAILD 0
#define STATUS_SUCCEED 1
#define STATUS_WAITTING 2
#define STATUS_PROCESSING 3
#define STATUS_FAILED 4

#define VIDEO_FRAME_DEFAULT_WIDTH 1920
#define VIDEO_FRAME_DEFAULT_HEIGHT 1080
...
int postData(LPVOID lpParam, int faceID);
...
BEGIN_MESSAGE_MAP(CArcFaceDemoDlg, CDialogEx)
	...
	ON_WM_CLOSE()
	ON_STN_CLICKED(IDC_STATIC_VIEW, &CArcFaceDemoDlg::OnStnClickedStaticView)
END_MESSAGE_MAP()
...
void CArcFaceDemoDlg::OnPaint()
{
	...
	else
	{
		if (m_videoOpened)
		{
			...
			if (m_curFaceDrawn.size() == 0)
			{
				CRect zrect(0, 0, 0, 0);
				IplDrawToHDC(TRUE, m_curVideoImage, zrect, IDC_STATIC_VIEW);
			}
			else
			{
				//文字显示框
				for (int i = 0; i < m_curFaceDrawn.size(); i++)
				{
					CRect rect(m_faceList[m_curFaceDrawn[i]].faceInfo.faceRect.left - 10, m_faceList[m_curFaceDrawn[i]].faceInfo.faceRect.top - 50,
						m_faceList[m_curFaceDrawn[i]].faceInfo.faceRect.right, m_faceList[m_curFaceDrawn[i]].faceInfo.faceRect.bottom);
					IplDrawToHDC(TRUE, m_curVideoImage, m_faceList[m_curFaceDrawn[i]].videoShowString, rect, IDC_STATIC_VIEW);
				}
			}
		}
		else
...
unsigned long _stdcall RunLoadThumbnailThread(LPVOID lpParam)
{
	...
	while (bWorking)
	{
		...
		if (IsImageGDIPLUSValid(m_strCurrentDirectory + _T("\\") + filePath))//是否是图片
		{
			...
			m_vFileTitle.push_back(finder.GetFileTitle());
		}
	}
		...
		iter++)
	{
		...
		//储存学号信息
		dialog->m_studentIDsVec.push_back(m_vFileTitle[allIndex]);
		...
		//显示学号 (char*)(LPCTSTR)(dialog->m_studentIdsVec[actualIndex])
		dialog->m_ImageListCtrl.InsertItem(actualIndex, (char*)(LPCTSTR)(dialog->m_studentIDsVec[actualIndex]), actualIndex);
...
void CArcFaceDemoDlg::OnBnClickedBtnCamera()
{
	...
	if (btnLabel == "启用摄像头")
	{
		...
		//一分钟后将当前画面中的考勤信息上传服务器
		m_postTime = time(0) + 60;
	}
	else
	{
		GetDlgItem(IDC_BTN_COMPARE)->EnableWindow(TRUE);
...
unsigned long _stdcall RunFaceDetectOperation(LPVOID lpParam)
{
	...
	if (g_cameraNum == 2)
	{
		...
		{
			bool res = rgbCapture.open(g_rgbCameraId) && irCapture.open(g_irCameraId);
			if (res)
				dialog->m_videoOpened = true;

			CString test = "";
			test.Format("网络摄像头初始化结果: %d", res);
			dialog->EditOut(test, TRUE);
		}
	...
	else if (g_cameraNum == 1)
	{
		if (!rgbCapture.isOpened())
		{
			...
			//填写网络摄像头的rtsp地址
			//********为摄像头密码，需进行替换
			bool res = rgbCapture.open("rtsp://admin:********@192.168.43.216:554/h264/ch1/main/av_stream");
			CString test = "";
			test.Format("网络摄像头初始化结果: %d", res);
			dialog->EditOut(test, TRUE);
			if (res)
	...
	while (dialog->m_videoOpened)
	{
		...
		else if (g_cameraNum == 1)
		{
			...
			MRESULT detectRes = dialog->m_videoFaceEngine.PreDetectFace(&rgbImage, multiFaceInfo, true);

			dialog->m_curFaceDrawn.clear();
			dialog->m_curFaceSet.clear();
			if (MOK == detectRes)
			{
				cvRectangle(&rgbImage, cvPoint(faceInfo.faceRect.left, faceInfo.faceRect.top),
					cvPoint(faceInfo.faceRect.right, faceInfo.faceRect.bottom), cvScalar(0, 0, 255), 2);
				...
				{
					//更新faceInfo
					int faceID = multiFaceInfo.faceID[i];
					faceInfo.faceRect.left = multiFaceInfo.faceRect[i].left;
					faceInfo.faceRect.top = multiFaceInfo.faceRect[i].top;
					faceInfo.faceRect.right = multiFaceInfo.faceRect[i].right;
					faceInfo.faceRect.bottom = multiFaceInfo.faceRect[i].bottom;
					faceInfo.faceOrient = multiFaceInfo.faceOrient[i];
					dialog->m_faceList[faceID].faceInfo = faceInfo;

					if (dialog->m_faceList[faceID].faceStatus == STATUS_INVAILD)
					{
						//记录人脸信息第一次出现的时间
						time_t beginTime = time(0);
						dialog->m_faceList[faceID].faceStatus = STATUS_WAITTING;
						dialog->m_faceList[faceID].begin = beginTime;
						dialog->m_FRList.push(faceID);
					}
					else if (dialog->m_faceList[faceID].faceStatus == STATUS_FAILED)
					{
						dialog->m_curFaceDrawn.push_back(faceID);
						dialog->m_faceList[faceID].faceStatus = STATUS_WAITTING;
						dialog->m_FRSecList.push(faceID);
					}
					else
					{
						dialog->m_curFaceDrawn.push_back(faceID);
						if (dialog->m_faceList[faceID].faceStatus == STATUS_SUCCEED)
						{
							dialog->m_curFaceSet.push_back(faceID);
						}
					}
				}

				sort(dialog->m_befFaceSet.begin(), dialog->m_befFaceSet.end());
				sort(dialog->m_curFaceSet.begin(), dialog->m_curFaceSet.end());
				vector<int> result;
				set_difference(
					dialog->m_befFaceSet.begin(), dialog->m_befFaceSet.end(),
					dialog->m_curFaceSet.begin(), dialog->m_curFaceSet.end(),
					back_inserter(result)
				);

				//对每个离开画面的人脸数据向服务器发送数据
				for (int i = 0; i < result.size(); i++)
				{
					CString res = "";
					int faceID = result[i];
					CString stuID = dialog->m_faceList[faceID].StudentID;
					int statusCode = postData(lpParam, faceID);
					switch (statusCode)
					{
					case -1:
						res.Format("%s：向服务器发送数据失败", stuID);
						dialog->EditOut(res, TRUE);
						break;
					case 20000:
						res.Format("%s：向服务器发送数据成功", stuID);
						dialog->EditOut(res, TRUE);
						break;
					case 40001:
						res.Format("%s：未查询到相关学生信息", stuID);
						dialog->EditOut(res, TRUE);
						break;
					case 40002:
						res.Format("%s：数据格式错误", stuID);
						dialog->EditOut(res, TRUE);
						break;
					default:
						break;
				...
				dialog->m_befFaceSet = dialog->m_curFaceSet;	//更新画面中的人脸信息

				time_t now = time(0);
				//每隔一段时间将当前画面中的考勤数据上传服务器
				if (now >= dialog->m_postTime)
				{
					for (int i = 0; i < dialog->m_curFaceSet.size(); i++)
					{
						CString res = "";
						int faceID = dialog->m_curFaceSet[i];
						CString stuID = dialog->m_faceList[faceID].StudentID;
						int statusCode = postData(lpParam, faceID);
						switch (statusCode)
						{
						...
						time_t newBegin = time(0);	//更新画面中人脸的初始进入时间
						dialog->m_faceList[faceID].begin = newBegin;
					}

					dialog->m_postTime += 60;	//设定下一次上传数据的时间为一分钟后
				}

				//FRList队列不为空
				if (!dialog->m_FRList.empty())
				{
					dialog->m_curFaceID = dialog->m_FRList.front();
					dialog->m_FRList.pop();
					dialog->m_curFaceInfo = dialog->m_faceList[dialog->m_curFaceID].faceInfo;
					dialog->m_dataValid = true;
				}
				//FRList队列为空时，处理FRSecList队列
				else
				{
					if (!dialog->m_FRSecList.empty())
					{
						dialog->m_curFaceID = dialog->m_FRSecList.front();
						dialog->m_FRSecList.pop();
						dialog->m_curFaceInfo = dialog->m_faceList[dialog->m_curFaceID].faceInfo;
						dialog->m_dataValid = true;
					}
					else
			...
			{
				//画面中没有人脸出现，将原先所有的人脸数据发送给服务器
				for (int i = 0; i < dialog->m_befFaceSet.size(); i++)
				{
					CString res = "";
					int faceID = dialog->m_befFaceSet[i];
					CString stuID = dialog->m_faceList[faceID].StudentID;
					int statusCode = postData(lpParam, faceID);
					switch (statusCode)
					{
...
unsigned long _stdcall RunFaceFeatureOperation(LPVOID lpParam)
{
	...
	dialog->m_faceList[dialog->m_curFaceID].faceStatus = STATUS_PROCESSING;
	...
	while (dialog->m_bFDThreadRunning)
	{
		...
		{
			//加载和清除注册库的过程中 不要显示信息
			dialog->m_faceList[dialog->m_curFaceID].videoShowString = "";
			continue;
		}
		...
		/*CString resStr = "";
		resStr.Format("正在检测：%d", dialog->m_curFaceID);
		dialog->EditOut(resStr, TRUE);*/
		...
		if (detectRes == 0 && liveNessInfo.num > 0)
		{
			...
			{
				dialog->m_faceList[dialog->m_curFaceID].videoShowString = "RGB假体";
				dialog->m_faceList[dialog->m_curFaceID].faceStatus = STATUS_FAILED;
			}
			else
			{
				//-1：不确定；-2:传入人脸数>1； -3: 人脸过小；-4: 角度过大；-5: 人脸超出边界 
				dialog->m_faceList[dialog->m_curFaceID].videoShowString = "unknown";
				dialog->m_faceList[dialog->m_curFaceID].faceStatus = STATUS_FAILED;
			}
		}
		else
		...
		if (!(isRGBAlive && isIRAlive))
		{
			if (isRGBAlive && !isIRAlive)
			{
				dialog->m_faceList[dialog->m_curFaceID].videoShowString = "RGB活体";
			}
		...
		{
			CString resStr;
			resStr.Format(" :%.2f", maxThreshold);
			dialog->m_faceList[dialog->m_curFaceID].videoShowString = dialog->m_studentIDsVec[maxIndex-1] + resStr + ",RGB活体";
			dialog->m_faceList[dialog->m_curFaceID].StudentID = dialog->m_studentIDsVec[maxIndex - 1];
			dialog->m_faceList[dialog->m_curFaceID].faceStatus = STATUS_SUCCEED;
			dialog->m_befFaceSet.push_back(dialog->m_curFaceID);
		}
		else if (isRGBAlive)
		{
			dialog->m_faceList[dialog->m_curFaceID].videoShowString = "RGB活体";
...
//双缓存画图重载
void CArcFaceDemoDlg::IplDrawToHDC(BOOL isVideoMode, IplImage* rgbImage, CString videoShowString, CRect& strShowRect, UINT ID)
{
	if (!rgbImage)
	{
		return;
	...
	if (isVideoMode)
	{
		...
		SIZE size;
		GetTextExtentPoint32A(pDc, videoShowString, (int)strlen(videoShowString), &size);
		...
		//画比对信息
		if (videoShowString == "RGB假体")
		{
			pDc.SetTextColor(RGB(255, 242, 0));
		}
		pDc.DrawText(videoShowString, &strShowRect, DT_TOP | DT_LEFT | DT_NOCLIP);

		if (videoShowString == "IR假体")
		{
			pDc.SetTextColor(RGB(255, 242, 0));
		}
		pDc.DrawText(videoShowString, CRect(20, 20, 100, 100), DT_TOP | DT_LEFT | DT_NOCLIP);
	}
	else
	{
		//图片由于尺寸不一致 ，需要重绘背景
...
void CArcFaceDemoDlg::OnStnClickedStaticView()
{
	// TODO: 在此添加控件通知处理程序代码
	if (!bFullScreen)
	{
		...
		GetDlgItem(IDC_STATIC_VIEW)->GetWindowPlacement(&m_struOldWndpPic);
		...
		//将PICTURE控件的坐标设为全屏大小
		GetDlgItem(IDC_STATIC_VIEW)->MoveWindow(CRect(0, 0, g_iCurScreenWidth, g_iCurScreenHeight));
	}
	else
	{
		GetDlgItem(IDC_STATIC_VIEW)->SetWindowPlacement(&m_struOldWndpPic);
...
int postData(LPVOID lpParam, int faceID)
{
	...
	time_t end = time(0);
	time_t begin = dialog->m_faceList[faceID].begin;
	int duration = difftime(end, begin);
	tm *ltm = localtime(&begin);
	int statusCode = -1;
	char postJsonData[1024];
	memset(postJsonData, 0, sizeof(postJsonData));
	...
	data["stu_id"] = dialog->m_faceList[faceID].StudentID.GetBuffer(0);
	data["begin_time"] = time;
	data["duration"] = duration;
	string strJson = writer.write(data);
	
	strcpy(postJsonData, strJson.c_str());
	try
	{
		CURL *pCurl = NULL;
		CURLcode res;
		...
		if (NULL != pCurl)
		{
			...
			curl_easy_setopt(pCurl, CURLOPT_URL, "http://139.224.232.165/api/setstate");
			...
			// 设置要POST的JSON数据
			curl_easy_setopt(pCurl, CURLOPT_POSTFIELDS, postJsonData);

			curl_easy_setopt(pCurl, CURLOPT_WRITEFUNCTION, write_data);//设置回调函数

			curl_easy_setopt(pCurl, CURLOPT_WRITEDATA, &out);//设置写数据
			...
			{
				CString failedInfo = "";
				failedInfo.Format("curl_easy_perform() failed:%s\n", curl_easy_strerror(res));
				dialog->EditOut(failedInfo, TRUE);
			}
			else
			{
				...
				if (read.parse(str_json, jsonObj))
				{
					statusCode = jsonObj["code"].asInt();
				}
			}
			// always cleanup
	...
	{
		CString exInfo;
		exInfo.Format("curl exception %s.\n", ex.what());
		dialog->EditOut(exInfo, TRUE);
	}
